...
const FALLBACK_RESULTS_CONTENT = {
  "Dry Hair": {
...
const FALLBACK_FAQ_CONTENT = [
  { question: "Is this safe for color-treated hair?", answer: "Yes! Our formula is sulfate-free and gentle, making it perfectly safe for all color-treated hair." },
  { question: "How often should I use this?", answer: "For best results, use 2-3 times a week or as often as you wash your hair." },
  { question: "Does it contain silicones?", answer: "No, we use natural alternatives to silicones that won't build up on your scalp." }
];

const FALLBACK_UGC_CONTENT = [
  { videoUrl: "https://example.com/video1.mp4", author: "Sarah M.", quote: "This completely transformed my dry hair!" },
  { videoUrl: "https://example.com/video2.mp4", author: "Elena R.", quote: "The best hydration my scalp has ever felt." }
];
...
let sql = `
-- 1. Ensure JSONB columns exist
ALTER TABLE public.products
ADD COLUMN IF NOT EXISTS education_content JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS clinical_results JSONB DEFAULT '{}'::jsonb,
ADD COLUMN IF NOT EXISTS faqs JSONB DEFAULT '[]'::jsonb,
ADD COLUMN IF NOT EXISTS ugc_videos JSONB DEFAULT '[]'::jsonb;

-- 2. Update existing products with data
UPDATE public.products
SET 
  education_content = '${escapeString(JSON.stringify(FALLBACK_EDUCATION_CONTENT))}'::jsonb,
  clinical_results = '${escapeString(JSON.stringify(FALLBACK_RESULTS_CONTENT))}'::jsonb,
  faqs = '${escapeString(JSON.stringify(FALLBACK_FAQ_CONTENT))}'::jsonb,
  ugc_videos = '${escapeString(JSON.stringify(FALLBACK_UGC_CONTENT))}'::jsonb
WHERE true; -- Applies to all current products to populate the mock data
