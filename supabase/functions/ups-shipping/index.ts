...
const validateAddress = async (address: UPSAddress): Promise<UPSAddress[]> => {
  ...
  if (data.XAVResponse?.ValidAddressIndicator) {
    ...
    const candidates =
      Array.isArray(data.XAVResponse.CandidateAddressList.CandidateAddress)
        ? data.XAVResponse.CandidateAddressList.CandidateAddress
        : [data.XAVResponse.CandidateAddressList.CandidateAddress];

    return candidates.map((candidate: any) => ({
      addressLine: candidate.AddressKeyFormat.AddressLine[0] ||
        address.addressLine,
...
// Get shipping rates from UPS - using Shop endpoint to get all available services at once
const getShippingRates = async (
  ...
  console.log(`Cache MISS for ${cacheKey}, fetching from UPS API (Shop)...`);

  const accessToken = await getUPSAccessToken();
  const accountNumber = Deno.env.get("UPS_ACCOUNT_NUMBER"); // Optionally store account number in env
  ...
  try {
    const ratePayload = {
      RateRequest: {
        ...
        Shipment: {
          ...
          // Enable Negotiated Rates if account number is provided
    ...
    console.log(
      `Requesting Shop rates to ${toAddress.countryCode} with weight ${weight}kg`,
    );
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error("UPS Shop API error:", errorText);
    ...
    if (data.RateResponse?.RatedShipment) {
      ...
      for (const shipment of shipments) {
        ...
        const negotiatedPrice = shipment.NegotiatedRateCharges?.TotalCharge
          ?.MonetaryValue;
        const totalPrice = parseFloat(
          negotiatedPrice || shipment.TotalCharges?.MonetaryValue || "0",
        );
    ...
    console.log(`Found ${rates.length} rates via Shop API`);
    ...
    if (rates.length > 0) {
      ...
      console.log(`Cached ${rates.length} rates for ${cacheKey}`);
    }

    return rates;
  } catch (err) {
    console.error("Error in getShippingRates (Shop API):", err);
    throw err;
  }
};
